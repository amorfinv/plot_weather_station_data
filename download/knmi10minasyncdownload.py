...
async def main():
    ...
    # get download directory and api key and other from subprocess call
    base_url = sys.argv[1]
    download_directory = sys.argv[2]
    api_key = sys.argv[3]
    cut_off_year = sys.argv[4]

    dataset_name = "windgegevens"
    dataset_version = "1.0"
    ...
    # delete anything before cutoff year
    filenames = [x for x in filenames if x >= f"kis_tow_{cut_off_year}01.gz"]
...
def list_dataset_files(
    ...
    if list_files_response.status_code != 200:
        print(list_files_response.text)
